...
object SudokuChecker {
    ...
    fun initGrid(context: Context, gridLayout: GridLayout, dimensions: Int, arr: Array<IntArray>) {
        cellDimensions = dimensions
        gridCell = initCell(context, arr)
        var i = 0
        var j: Int
        for (a in 0..10) {
            j = 0
    ...
    private fun initCell(context: Context, a: Array<IntArray>): Array<Array<EditText?>> {
        ...
        for (i in 0..8) {
            for (j in 0..8) {
                ...
                if (a[i][j] != 0) {
                    sudokuCell[i][j]!!.setText(a[i][j].toString())
                }
    ...
    fun getCellValues(): Array<IntArray> {
        val x = Array(9) { IntArray(9) }
        for (i in 0..8) {
            for (j in 0..8) {
    ...
    val solution: Boolean
        get() {
            ...
            for (i in 0..8) {
                for (j in 0..8) {
                    Log.d(ContentValues.TAG, cellValues[i][j].toString())
                }
            }
    ...
    fun updateSolution(): Array<IntArray> {
        val result = Array(9) { IntArray(9) }
        for (i in 0..8) {
            for (j in 0..8) {
                result[i][j] = cellValues[i][j]
            }
        }
        return result
    }
