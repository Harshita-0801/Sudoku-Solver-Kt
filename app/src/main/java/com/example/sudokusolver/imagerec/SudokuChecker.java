...
public class SudokuChecker {
    ...
    private static int[][] copygrid = new int[9][9];
    private static int cellDimensions;

    public static void initGrid(Context context, GridLayout gridLayout, int dimensions, int[][] arr) {
    ...
    private static EditText[][] initCell(Context context, int a[][]) {
        ...
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                ...
                if (a[i][j] != 0) {
                    sudokuCell[i][j].setText(String.valueOf(a[i][j]));
                }
    ...
    public static void updateGrid() {
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                gridCell[i][j].setText(String.valueOf(copygrid[i][j]));
    ...
    public static boolean getSolution() {
        ...
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                Log.d(TAG,String.valueOf(cellValues[i][j]));
            }
        }
